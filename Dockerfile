...
RUN echo "Cloning base repository into $WORK_DIR/seiscomp" \
    && git clone $REPO_PATH/seiscomp.git $WORK_DIR/seiscomp \
    && echo "Cloning base components" \
    && cd $WORK_DIR/seiscomp/src/base \
    && git clone $REPO_PATH/seedlink.git \
    && git clone $REPO_PATH/common.git \
    && sed -i '1105s/AlternateBase/Mid/;1269s/AlternateBase/Mid/' common/libs/seiscomp/gui/datamodel/originlocatorview.cpp \
